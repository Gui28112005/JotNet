...
export default function BlocoDeNotas() {
  ...
  const carregarNotasSalvas = async () => {
    try {
      const notasArmazenadas = await AsyncStorage.getItem('@notasTela');
  ...
  const salvarNotas = async (notas) => {
    try {
      await AsyncStorage.setItem('@notasTela', JSON.stringify(notas));
    } catch (error) {
  ...
  const renderItem = ({ item }) => (
    <View style={styles.item}>
      ...
      <TouchableOpacity onPress={() => removerNota(item.id)} style={styles.botaoRemover}>
        <Text style={styles.botaoRemoverTexto}>Remover Nota</Text>
      </TouchableOpacity>
    </View>
  );
  ...
  return (
    <LinearGradient colors={['#78ad89', 'black']} style={styles.container}>
      ...
      <TextInput
        style={styles.input}
        placeholder="Digite suas anotação..."
        value={novaNota}
