...
export default function BlocoDeNotas() {
  ...
  const carregarNotasSalvas = async () => {
    try {
      const notasArmazenadas = await AsyncStorage.getItem('@notasMelhoreCinemas'); // Chave única para esta tela
  ...
  const salvarNotas = async (notas) => {
    try {
      await AsyncStorage.setItem('@notasMelhoreCinemas', JSON.stringify(notas)); // Chave única para esta tela
    } catch (error) {
      console.error('Erro ao salvar Cinemas:', error);
    }
  };
  ...
  const renderItem = ({ item }) => (
    <View style={styles.item}>
      ...
      <TouchableOpacity onPress={() => removerNota(item.id)} style={styles.botaoRemover}>
        <Text style={styles.botaoRemoverTexto}>Remover cinema</Text>
      </TouchableOpacity>
    </View>
  );
  ...
  return (
    <LinearGradient colors={['#78ad89', 'black']} style={styles.container}>
      ...
      <Text style={styles.titulo}>Cinemas</Text>
      <TextInput
        style={styles.input}
        placeholder="Digite seu melhor Cinema..."
        value={novaNota}
        onChangeText={text => setNovaNota(text)}
      />
      <TouchableOpacity onPress={adicionarNota} style={styles.botao}>
        <Text style={styles.botaoTexto}>Adicionar Cinemas</Text>
      </TouchableOpacity>
      <FlatList
        data={notas}
