...
export default function BlocoDeNotas() {
  ...
  const carregarNotasSalvas = async () => {
    try {
      const notasArmazenadas = await AsyncStorage.getItem('@notasMelhoresRestaurantes'); // Chave única para esta tela
  ...
  const salvarNotas = async (notas) => {
    try {
      await AsyncStorage.setItem('@notasMelhoresRestaurantes', JSON.stringify(notas)); // Chave única para esta tela
    } catch (error) {
  ...
  const renderItem = ({ item }) => (
    <View style={styles.item}>
      ...
      <TouchableOpacity onPress={() => removerNota(item.id)} style={styles.botaoRemover}>
        <Text style={styles.botaoRemoverTexto}>Remover Restaurante</Text>
      </TouchableOpacity>
    </View>
  );
  ...
  return (
    <LinearGradient colors={['#78ad89', 'black']} style={styles.container}>
      ...
      <TextInput
        style={styles.input}
        placeholder="Digite seu melhor Restaurante..."
        value={novaNota}
        onChangeText={text => setNovaNota(text)}
      />
      <TouchableOpacity onPress={adicionarNota} style={styles.botao}>
        <Text style={styles.botaoTexto}>Adicionar Restaurante</Text>
      </TouchableOpacity>
      <FlatList
        data={notas}
